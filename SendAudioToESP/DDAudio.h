...
// Variables to adjust beat detecting
const int timeElapsedThreshold = 250; // This variable is the amount of time that needs to pass between beats that are detected, in milliseconds
const double stDevMultiplier = 2.3; // This variable is multiplied with stDev and added to average, which is the threshold for detecting beats
...
//  if (button1.pressed()) myInput = AUDIO_INPUT_MIC;
//  else myInput = AUDIO_INPUT_LINEIN;
...
void beatTiming(int i) {
  if (beatDetected[i] == 2) { // if a new beat has been detected
    if (beatTimer[i] < timeElapsedThreshold) // but it is less than 200ms from the last one
...
void beatDetection(int i) {
  ...
  // 2 = new beat detected
  // Peak detection is performed here. FFT value must be greater than the average+(2.5*st.dev) and greater than a threshold of 300, which is pretty quiet
  if (spectrumValue[i] > average[i] + stDevMultiplier * stDev[i] && spectrumValue[i] > 300) {
...
void printDetectedBeats(int i) {
  ...
  if (i == 13) Serial.println(beatDetected[i]);
  else {
...
void musicAnalytics() {
  ...
  if (beatDetected[lowBeatBin] == 2 || constantBeatCounter[lowBeatBin] > 8 || beatDetected[2] == 2)
